...
export default function WindowHeader({
  ...
  const { drag, isMaximized, isMinimized, minimize, maximize, restore } = useWindowUI();

  const isMobile = useIsMobile(MOBILE_BREAKPOINT);

  return (
    <div className={`window-header ${styles.header}`} onMouseDown={drag}>
      <span className={`window-title ${brandStyles.title}`}>
        {icon && (
          <span className={`window-icon ${brandStyles.windowIcon} ${brandStyles.brandText}`}>
            {icon}
          </span>
        )}
        <span className={`brand-text ${brandStyles.brandText}`}>{title}</span>
      </span>
      ...
      <div className={`window-controls ${controlStyles.controls}`}>
        <MinimizeButton
          ...
          disabled={canMinimize === false}
        />
        <MaximizeButton
          onClick={isMaximized ? restore : maximize}
          isMaximized={isMaximized}
          disabled={canMaximize === false || isMobile}
        />
